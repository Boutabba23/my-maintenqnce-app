...
import { PlusIcon, TrashIcon, PencilIcon, SearchIcon, EllipsisVerticalIcon, EyeIcon, WrenchIcon } from '../constants';
...
export const MaintenanceView: React.FC<MaintenanceViewProps> = ({ maintenanceRecords, machines, onOpenMaintenanceModal, onDeleteMaintenance, onOpenConfirmationDialog }) => {
  ...
  const handleDeleteClick = (record: MaintenanceRecord) => {
    onOpenConfirmationDialog({
        ...
        description: `Êtes-vous sûr de vouloir supprimer cette opération de maintenance pour ${getMachineName(record.machineId)} ? Cette action est irréversible.`,
  ...
  const filteredMaintenanceRecords = useMemo(() => {
    ...
    if (rangeFilter !== 'all') {
      records = records.filter(record => record.maintenanceRange === rangeFilter);
    }
    ...
    if (searchQuery) {
        ...
        records = records.filter(record => {
            ...
            const machineIdentifier = machine ? `${machine.designation} ${machine.code}`.toLowerCase() : '';
  ...
  const getNextMaintenanceInfo = (record: MaintenanceRecord): { gamme: string; hours: number } => {
    const machineHistory = machineMaintenanceHistory.get(record.machineId);
    if (!machineHistory) return { gamme: '?', hours: 0 };

    const recordIndexInHistory = machineHistory.findIndex(r => r.id === record.id);
    ...
    const nextSequenceIndex = masterSequencePointer % maintenanceSequence.length;
  ...
  const renderContent = () => {
    ...
    if (filteredMaintenanceRecords.length === 0 && (searchQuery || machineFilter !== 'all' || rangeFilter !== 'all')) {
        return (
            <EmptyState
                ...
                action={
                    <Button variant="outline" onClick={() => { setSearchQuery(''); setMachineFilter('all'); setRangeFilter('all'); }}>
                        Effacer les filtres
                    </Button>
                }
            />
    ...
    const recordsToRender = [...filteredMaintenanceRecords].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    
    const actionMenuItems = (record: MaintenanceRecord) => (
      <ul className="py-1">
          <li>
              <button 
                  onClick={(e) => { e.stopPropagation(); onOpenMaintenanceModal(record, true); setOpenMenuId(null); }}
          ...
          <li>
              <button 
                  onClick={(e) => { e.stopPropagation(); onOpenMaintenanceModal(record); setOpenMenuId(null); }}
                  className="w-full text-left flex items-center px-4 py-2 text-sm text-foreground hover:bg-muted"
              >
          ...
          <li>
              <button 
                  onClick={(e) => { e.stopPropagation(); handleDeleteClick(record); setOpenMenuId(null); }}
              ...
              <Card key={record.id}>
                <CardHeader className="pb-2">
                  <div className="flex justify-between items-start">
                    <div>
                      <CardTitle className="text-lg">{getMachineName(record.machineId)}</CardTitle>
                    ...
                    <div className="relative menu-container flex-shrink-0">
                      <Button variant="ghost" size="icon" className="-mr-2 -mt-2" onClick={(e) => { e.stopPropagation(); setOpenMenuId(openMenuId === record.id ? null : record.id); }}>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-x-4 gap-y-6 pt-4 border-t">
                    <div>
                      <p className="text-sm text-muted-foreground">Heures de Service</p>
                      <p className="font-semibold">{record.serviceHours.toLocaleString()} hs</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">Filtres Changés</p>
                      <p className="font-semibold">{record.filtersUsed.length} filtre(s)</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground mb-1">Gamme Exécutée</p>
                      <span className={`font-bold inline-block text-center w-8 h-8 leading-8 rounded-full text-sm ${rangeColors[record.maintenanceRange]}`}>
                        {record.maintenanceRange}
                      </span>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground mb-1">Gamme Prochaine</p>
                      <div className="flex items-center gap-2">
                        <span className={`font-bold inline-block text-center w-8 h-8 leading-8 rounded-full text-sm ${rangeColors[nextMaint.gamme as keyof typeof rangeColors]}`}>
                          {nextMaint.gamme}
                        </span>
                        <span className="text-sm font-semibold">{nextMaint.hours.toLocaleString()} hs</span>
                      </div>
                    </div>
                  </div>
                </CardContent>
        ...
        <div className="hidden md:block">
            <Card>
                <CardContent className="p-0">
                    <Table className="table-zebra" wrapperClassName="rounded-lg" isMenuOpen={!!openMenuId}>
                        <TableHeader>
                            <TableRow>
                        ...
                        <TableBody>
                            ...
                            return (
                                <TableRow key={record.id}>
                                    <TableCell className="font-medium max-w-sm truncate">{getMachineName(record.machineId)}</TableCell>
                                    <TableCell>{new Date(record.date).toLocaleDateString()}</TableCell>
                                    <TableCell>{record.serviceHours.toLocaleString()} hs</TableCell>
                                    <TableCell>
                                        <span className={`font-bold inline-block text-center w-8 h-8 leading-8 rounded-full text-xs ${rangeColors[record.maintenanceRange]}`}>
                                            {record.maintenanceRange}
                                        </span>
                                    </TableCell>
                                    <TableCell>
                                    <div className="flex items-center gap-2">
                                        <span className={`font-bold inline-block text-center w-8 h-8 leading-8 rounded-full text-xs ${rangeColors[nextMaint.gamme as keyof typeof rangeColors]}`}>
                                            {nextMaint.gamme}
                                        </span>
                                        <span className="text-xs text-muted-foreground">{nextMaint.hours.toLocaleString()} hs</span>
                                    </div>
                                    </TableCell>
                                    ...
                                    <TableCell className="text-right">
                                        <div className="relative menu-container flex justify-end">
                                            <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); setOpenMenuId(openMenuId === record.id ? null : record.id); }}>
                                                <EllipsisVerticalIcon />
                                            </Button>
                                            {openMenuId === record.id && (
                                                <div className={`absolute right-0 w-48 bg-card border border-border rounded-md shadow-lg z-50 ${isLastRow ? 'bottom-full mb-2' : 'top-full mt-2'} animate-fadeIn`}>
                                                   {actionMenuItems(record)}
                                                </div>
                                            )}
                                        </div>
      ...
      <Card className="mb-6">
        <CardHeader>
          <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
            <div>
              <CardTitle className="text-2xl">Suivi de la Maintenance Préventive</CardTitle>
              <CardDescription>
                Consultez et enregistrez les opérations de maintenance de votre parc.
              </CardDescription>
            </div>
            <Button onClick={() => onOpenMaintenanceModal(null)} className="flex-shrink-0">
              <PlusIcon className="mr-2 h-5 w-5" />
            ...
            <CardContent>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
                    <div className="space-y-2">
                        ...
                        <Select id="machine-filter" value={machineFilter} onChange={e => setMachineFilter(e.target.value)}>
                            <option value="all">Tous les engins</option>
                            {machines.map(m => <option key={m.id} value={m.id}>{m.designation}</option>)}
                        </Select>
                    </div>
                    <div className="space-y-2">
                        <Label htmlFor="range-filter">Gamme</Label>
                        <Select id="range-filter" value={rangeFilter} onChange={e => setRangeFilter(e.target.value)}>
