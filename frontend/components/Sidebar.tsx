    ...
    <>
        ...
        <aside className={`fixed top-0 left-0 z-40 w-64 h-screen transition-transform transform ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 bg-card border-r border-border flex flex-col`}>
          ...
          <nav className="flex-1 p-4 space-y-2">
            <NavItem view={View.DASHBOARD} label="Tableau de bord" icon={<Squares2x2Icon className="h-5 w-5"/>} />
            ...
            <NavItem view={View.MAINTENANCE} label="Maintenance Préventive" icon={<WrenchIcon className="h-5 w-5"/>} />
            <NavItem view={View.ANALYTICS} label="Analyses" icon={<ChartBarIcon className="h-5 w-5"/>} />
