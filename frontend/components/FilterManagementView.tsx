...
import { FilterGroup, StockUpdateInfo, FilterReference } from '../types';
import Button from './ui/Button';
import { PlusIcon, PencilIcon, TrashIcon, SearchIcon, EllipsisVerticalIcon, FilterIcon, ArrowDownTrayIcon, QrCodeIcon } from '../constants';
...
const FilterManagementView: React.FC<FilterManagementViewProps> = (props) => {
  ...
  const [qrCodeModalGroup, setQrCodeModalGroup] = useState<FilterGroup | null>(null);
  ...
  const handleAddStockClick = (e: React.MouseEvent, filterReference: FilterReference, filterGroupId: string) => {
    e.stopPropagation();
  ...
  const renderContent = () => {
    ...
    if (filteredFilterGroups.length === 0 && searchQuery) {
        return (
            <EmptyState
                icon={<SearchIcon className="h-24 w-24" />}
                title="Aucun résultat"
                description={`Aucun groupe de filtres ne correspond à votre recherche pour "${searchQuery}".`}
                action={
        ...
        <div className="space-y-6">
            {filteredFilterGroups.map(group => {
              const originalReference = group.originalReferenceId
                ? group.references.find(ref => ref.id === group.originalReferenceId)
                : null;
                ...
                >
                    ...
                    <div className="flex justify-between items-start gap-2">
                        ...
                        <div className="relative menu-container flex-shrink-0 flex items-center">
                            <Button variant="ghost" size="icon" className="-mr-2 -mt-2" onClick={(e) => { e.stopPropagation(); setQrCodeModalGroup(group); }}>
                                <QrCodeIcon className="h-5 w-5" />
                            </Button>
                            <Button variant="ghost" size="icon" className="-mr-2 -mt-2" onClick={(e) => { e.stopPropagation(); setOpenGroupMenuId(openGroupMenuId === group.id ? null : group.id); }}>
                            ...
                            {openGroupMenuId === group.id && (
                                <div className="absolute right-0 mt-2 w-48 bg-card border border-border rounded-md shadow-lg z-50 animate-fadeIn">
                                    <ul className="py-1">
                                        <li>
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); props.onOpenFilterGroupModal(group); setOpenGroupMenuId(null); }}
                                        ...
                                        <li>
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); handleDeleteClick(group); setOpenGroupMenuId(null); }}
                    ...
                    <CardContent>
                                ...
                                <TableBody>
                                  <TableRow key={originalReference.id}>
                                    ...
                                    <TableCell className="text-right">
                                        <Button size="sm" variant="outline" onClick={(e) => handleAddStockClick(e, originalReference, group.id)}>
                                            <PlusIcon className="mr-1 h-4 w-4" /> Ajouter Stock
                                        </Button>
                                    </TableCell>
                                  </TableRow>
                                </TableBody>
                      ...
                      {compatibleReferences.length > 0 ? (
                              ...
                              <TableBody>
                                  ...
                                  <TableRow key={ref.id}>
                                      ...
                                      <TableCell><Badge variant={getStockBadgeVariant(ref.stock)}>{ref.stock}</Badge></TableCell>
                                       <TableCell className="text-right">
                                            <Button size="sm" variant="outline" onClick={(e) => handleAddStockClick(e, ref, group.id)}>
                                                <PlusIcon className="mr-1 h-4 w-4" /> Ajouter Stock
                                            </Button>
                                       </TableCell>
                                  </TableRow>
                          ...
                          <p className="text-sm text-muted-foreground italic">
                            {originalReference ? 'Aucune autre référence compatible ajoutée.' : 'Aucune référence ajoutée à ce groupe.'}
                          </p>
                      )}
                    </CardContent>
                </Card>
              ...
              <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                  ...
                  <div className="flex items-center flex-shrink-0 gap-2">
                        {props.filterGroups.length > 0 && (
                            <Button onClick={() => exportFiltersToCSV(props.filterGroups)} variant="outline">
      ...
      {props.filterGroups.length > 0 && (
        <Card className="mb-6">
            <CardHeader>
                ...
                <CardDescription>
                    Filtrez par nom de groupe ou référence de filtre.
                </CardDescription>
            </CardHeader>
            <CardContent>
